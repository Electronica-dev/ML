...
class CausalSelfAttention(nn.Module):
    ...
    def __init__(self, config):
        ...
        # not really a 'bias', more of a mask but following the OAI/HF naming though
    ...
    def forward(self, x):
        ...
        # masking so that tokens cannot see the future
        att = att.masked_fill(self.bias[:,:,:T,:T] == 0, float("-inf"))
        # softmax so the probabilities sum to 1
...
class GPTConfig:
    ...
    vocab_size: int = 50257 # no. of tokens: 50k BPE merges + 256 byte tokens + 1 <|endoftext|>
    ...
    def from_pretrained(cls, model_type):
        ...
        for k in sd_keys_hf:
            if any(k.endswith(w) for w in transposed):
                ...
                # vanilly copy over the other parameters
...
while x.size(1) < max_length:
    ...
    with torch.no_grad():
        ...
        # take the logits at the last position; being wasteful as only the last position logits are used
        logits = logits[:, -1, :] # (B, vocab_size)
        # get the probabilities
        probs = F.softmax(logits, dim=-1)
        # do top-k sampling of 50 (huggingf ace pipeline default)
